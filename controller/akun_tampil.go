...
func Akun_tampil(c *gin.Context) {
	...
	var akun []user
	db.Raw("SELECT id, email, password, level FROM capstone.users WHERE level=? OR level=?", "perawat", "dokter").Scan(&akun)
	for i := 0; i < len(akun); i++ {
		if akun[i].Level == "perawat" {
			var perawat model.Perawat
			db.Raw("SELECT sip, nama_perawat, jenis_kelamin FROM capstone.perawats WHERE id_user=?;", akun[i].Id).Scan(&perawat)
		...
		if akun[i].Level == "dokter" {
			...
			db.Raw("SELECT sip, nama_dokter, jenis_kelamin FROM capstone.dokters WHERE id_user=?;", akun[i].Id).Scan(&dokter)
